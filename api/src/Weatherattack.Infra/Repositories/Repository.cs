...
namespace WeatherAttack.Infra.Repositories
{
        ...
        where Db: DbContext
    {
        ...
        public virtual async Task<IList<TResult>> GetAsync<TResult>(Expression<Func<Entity, TResult>> selector)
            => await RawGet().Select(selector).ToListAsync();

        public virtual async Task<IList<TResult>> GetAsync<TResult>(Expression<Func<Entity, bool>> predicate, Expression<Func<Entity, TResult>> selector)
            => await RawGet().Where(predicate).Select(selector).ToListAsync();

        public virtual async Task<IList<TResult>> GetAsync<TResult>(int skip, int take, Expression<Func<Entity, TResult>> selector)
            => await RawGet().Skip(skip).Take(take).Select(selector).ToListAsync();

        public virtual async Task<IList<TResult>> GetAsync<TResult>(int skip, int take, Expression<Func<Entity, bool>> predicate, Expression<Func<Entity, TResult>> selector)
        ...
        public virtual async Task<TResult> FindAsync<TResult>(long primaryKey, Expression<Func<Entity, TResult>> selector)
            => await RawGet().Where(e => e.Id == primaryKey).Select(selector).SingleOrDefaultAsync();

        public virtual async Task<TResult> FindAsync<TResult>(Expression<Func<Entity, bool>> predicate, Expression<Func<Entity, TResult>> selector)
            => await RawGet().Where(predicate).Select(selector).SingleOrDefaultAsync();
